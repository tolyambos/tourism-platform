...
interface ProductCard {
  ...
  oldPrice?: string;
  currency: string;
  priceType?: 'per_person' | 'per_group' | 'per_vehicle';
  startsFrom?: boolean;
  duration: string;
  images: string[];
  badges?: string[];
    ...
    <section id="featured-experiences" className="py-6 bg-gray-50 md:py-12">
      <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div className="mb-4 text-center md:mb-8">
          ...
          <h2 className="mb-2 text-2xl font-bold text-gray-900 md:text-3xl">{content.sectionTitle}</h2>
          <p className="text-gray-600">{content.sectionDescription}</p>
        </div>

        {/* Featured Recommendation Widget */}
        {content.featuredWidget?.enabled && (
          <div className="mx-auto max-w-5xl mb-12">
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 md:p-8 shadow-sm">
              <div className="flex items-center gap-2 mb-6 justify-center">
                <svg className="w-5 h-5 text-purple-600" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                </svg>
                <h3 className="text-lg font-bold text-gray-900">{content.featuredWidget.title || 'Recommended Experience'}</h3>
              </div>
              
              {/* GetYourGuide Widget Placeholder */}
              <div className="bg-white rounded-lg p-4 text-center text-gray-500">
                <p className="text-sm">GetYourGuide Widget - Tour ID: {content.featuredWidget.tourId}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="mx-auto space-y-6 max-w-5xl">
          {content.productCards.map((card, cardIndex) => (
                  ...
                  <div className="flex flex-col h-full">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex-1">
                        <div className="flex flex-wrap gap-2 items-center mb-2">
                          <div className="flex gap-2 items-center">
                            <span className="px-2 py-0.5 text-xs font-bold text-white bg-pink-600 rounded sm:text-sm">★ {card.rating}</span>
                            <span className="text-xs text-gray-600 sm:text-sm">({card.reviewCount} reviews)</span>
                          </div>
                          {card.duration && (
                            <div className="flex gap-2 items-center">
                      ...
                      <div className="ml-2 text-right sm:ml-4">
                        <div className="text-xl font-bold text-purple-600 sm:text-2xl">
                          {card.oldPrice && (
                            <>
                              <span className="text-gray-400 line-through text-base">{card.currency}{card.oldPrice}</span>
                              <span> {card.currency}{card.price}</span>
                            </>
                          )}
                          {!card.oldPrice && (
                            <>
                              {card.startsFrom && <span className="text-sm font-medium text-gray-600">From</span>}
                              <span> {card.currency}{card.price}</span>
                            </>
                          )}
                        </div>
                        <p className="text-xs text-gray-500 whitespace-nowrap">
                          {card.priceType === 'per_person' && 'per person'}
                          {card.priceType === 'per_group' && 'per group'}
                          {card.priceType === 'per_vehicle' && 'per vehicle'}
                          {!card.priceType && 'per person'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-auto">
                      ...
                      <a 
                        ...
                        className="px-6 py-2.5 text-sm font-semibold text-white bg-purple-600 rounded-lg transition-colors hover:bg-purple-700"
                      >
                        Book Now
                      </a>
                    </div>
