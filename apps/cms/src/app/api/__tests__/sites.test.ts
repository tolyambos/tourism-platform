  ...
  describe("GET /api/sites", () => {
    it("should return paginated sites list", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/sites?page=1&limit=10"
      );
    ...
    it("should filter sites by status", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/sites?status=PUBLISHED"
      );
      await GET(request);
    ...
    it("should search sites by name or subdomain", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/sites?search=rome"
      );
      await GET(request);
  ...
  describe("POST /api/sites", () => {
    ...
    it("should return 400 if subdomain already exists", async () => {
      vi.mocked(prisma.site.findUnique).mockResolvedValue({
        id: "existing",
      } as Awaited<ReturnType<typeof prisma.site.findUnique>>);
